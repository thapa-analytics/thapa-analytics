# 👋 Hi, I’m Ganesh Thapa

I am an **Agricultural Economist** with the **Wisconsin Department of Natural Resources (WDNR)**, specializing in applied microeconomics, spatial econometrics, and geospatial data science for policy evaluation.

My work integrates large administrative datasets, household surveys, and high-resolution environmental monitoring data to quantify how ecological conditions and regulatory interventions affect economic outcomes — including wildlife recreation participation, property markets, public health compliance costs, and regional economic activity.

## 🔍 Research & Policy Focus- at Wisconsin Department of Natural Resources

- **Wildlife Economics & Recreation Demand**
  - Chronic Wasting Disease (CWD) impacts on deer hunting participation and license revenue  
  - Waterfowl and upland bird hunting economic contributions  
  - IMPLAN-based regional impact modeling  

- **Environmental & Resource Economics**
  - Capitalization of lake-level fluctuations into waterfront housing prices  
  - Water quality and recreational amenity valuation  
  - Regulatory cost and benefit analyses 

- **Spatial & Econometric Methods**
  - Fixed-effects and staggered DiD designs (Callaway & Sant’Anna)
  - Spatial Durbin, SAC/SAR (Lee & Yu bias correction)
  - Hedonic pricing models
  - Interrupted time series analysis
 
  - ## 🛠 Technical Toolkit

**Languages:**  
`Python` · `R` · `Stata` · `SQL` · `ArcPy`

**Core Libraries & Tools:**  
`pandas` · `geopandas` · `statsmodels` · `spreg` · `shapely` · `USGS NWIS API` · `IMPLAN` · `ArcGIS Pro` · `Tableau`

**Data Sources:**  
`GoWild license sales` · `USGS gage data` · `NOAA/Daymet climate` ·  
`Census TIGER/ACS` · `WDNR SWIMS water-quality database`

## 📂 Featured Projects

- **🦌 CWD-Spatial-Econometrics**  
  *County-level panel analysis of disease spillovers and hunting participation.*

- **🏠 Lake-Level-Housing-Hedonics**  
  *High-frequency hydrology × housing transaction merges to estimate capitalization effects.*

- **🦆 Wisconsin-Bird-Hunting-IMPLAN**  
  *Input-output modeling of wildlife recreation expenditures and employment impacts.*

- **🗺 GIS-Automation-ArcPy**  
  *Production scripts for parcel–lake buffering, watershed matching, and spatial joins.*

## 📄 Research work undergoing in Wisconsin DNR

- *Localized and spillover effects of chronic wasting disease on license sales and hunter participation in Wisconsin* (Manusript ready for submission)
- *Chronic Wasting Disease and Hunter Behavior in the Upper Midwest: Assessing the Statewide Impact of CWD Detection on Hunting License Sales and Participation in Wisconsin* (Journal under review)
- *Hunting contributions to the State economy and wildlife conservation: evidence from ruffed grouse hunter expenditures in Wisconsin* (Journal under review)
- *Lake-Level Fluctuations and Waterfront Home Prices* (Manuscript ready for submission)  
- **Socio-economic Analysis – Enbridge Line 5 EIS**, Wisconsin DNR (2024) (Published report)  

## 📚 Recent Research & Publications

### 2025
- **Determinants of Use of Fertilizer Best Management Practices: Evidence from Smallholder Farms in Nepal**  
  *Experimental Agriculture*.  
  Examines adoption of 4R fertilizer practices using farm-level econometric models. Highlights roles of extension, demonstrations, and market access.  
  🔗 https://www.researchgate.net/profile/Ganesh-Thapa-2

- **Technology and Managerial Gaps in Adoption of Innovation: The Case of Ethiopian Wheat Farmers**  
  *Mishra, Thapa, Mottaleb, Fantaye* — Empirical Economics 69(1):1-37.  
  Productivity decomposition distinguishing technology gaps from management constraints among wheat farmers in Ethiopia.

### 2024
- **Fertilizer Demonstration, Agricultural Performance, and Food Security of Smallholder Farmers: Empirical Evidence from Nepal**  
  Evaluates on-farm fertilizer demonstrations using treatment-effects models.
    *Thapa, Choudhary, Pandit, Dongol* — World Development Sustainability 6(2):100196.  
   🔗 https://www.researchgate.net/publication/386541776

- **Enhancing Food Security in an Era of Rising Fertilizer Prices: Evaluation of an Intervention Promoting Mungbean Adoption in Nepal**  
  *Khanal, Choudhary, Thapa — Asian Development Review 42(1)*.  
  Food-security and income gains from legume adoption.  
  🔗 https://www.researchgate.net/publication/385590876

- **Global Fertilizer Crisis and Willingness to Pay for Chemical Fertilizers: Empirical Evidence from Nepal**
    *Thapa, Gaihre, Choudhary — Journal of Agribusiness in Developing and Emerging Economies*.  
    Estimates fertilizer WTP under price shocks using CVM and demand models.

### 2023
- **Does Private Sector Involvement Improve the Distribution Efficiency of Subsidized Fertilizer?**
   *Thapa, Gaihre, Choudhary, Gautam — Agricultural Economics*.  
  Natural-experiment evidence on improved fertilizer delivery efficiency.

- **Can traders' meetings boost seed sales of new rice varieties and promote a market-driven seed system? Evidence from Nepal**
   *Thapa, Gaihre, Choudhary, Khanal, Gautam — Journal of Agribusiness in Developing and Emerging Economies*.  
  Randomized outreach intervention showing gains in private seed diffusion.

### 2021
- **Adoption and diffusion of improved technologies and production practices in agriculture: Insights from a donor-led intervention in Nepal**  
  Large donor-program evaluation using quasi-experimental design.

## 📫 Contact

**Ganesh Thapa, PhD**  
 Economist — Wisconsin DNR  
📧 ganesh.thapa@wisconsin.gov
📧 thapaganesh@gmail.com  


















